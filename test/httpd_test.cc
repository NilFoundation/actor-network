...
SEASTAR_TEST_CASE(test_routes) {
    ...
    auto f1 = route.handle("/api", std::move(req), std::move(rep)).then([](std::unique_ptr<reply> rep) {
    ...
    req.reset(new request);
    rep.reset(new reply);
    auto f3 = route.handle("/api/abc", std::move(req), std::move(rep)).then([](std::unique_ptr<reply> rep) {});
    req.reset(new request);
    rep.reset(new reply);
    auto f4 = route.handle("/ap", std::move(req), std::move(rep)).then([](std::unique_ptr<reply> rep) {
...
SEASTAR_TEST_CASE(test_json_path) {
    ...
    auto f1 = route->handle("/my/path/value1/text", std::make_unique<request>(), std::make_unique<reply>())
                  .then([res1, route](auto f) { BOOST_REQUIRE_EQUAL(*res1, true); });

    auto f2 = route->handle("/my/path/value2/text1", std::make_unique<request>(), std::make_unique<reply>())
                  .then([res2, route](auto f) { BOOST_REQUIRE_EQUAL(*res2, true); });

    auto f3 = route->handle("/my/path/value3/text2/text3", std::make_unique<request>(), std::make_unique<reply>())
                  .then([res3, route](auto f) { BOOST_REQUIRE_EQUAL(*res3, true); });

    return when_all(std::move(f1), std::move(f2), std::move(f3)).then([](std::tuple<future<>, future<>, future<>> fs) {
        std::get<0>(fs).get();
        std::get<1>(fs).get();
        std::get<2>(fs).get();
    });
...
public:
    memory_data_sink(std::stringstream &ss) : data_sink(std::make_unique<memory_data_sink_impl>(ss)) {
    }
};
...
future<> test_transformer_stream(std::stringstream &ss, content_replace &cr, std::vector<sstring> &&buffer_parts) {
    ...
    return do_with(
        output_stream<char>(
            cr.transform(std::move(req), "json", output_stream<char>(memory_data_sink(ss), 32000, true))),
        std::vector<sstring>(std::move(buffer_parts)), [](output_stream<char> &os, std::vector<sstring> &parts) {
            return do_for_each(parts, [&os](auto &p) { return os.write(p); }).then([&os] { return os.close(); });
        });
}
    ...
    return do_with(std::stringstream(), content_replace("json"), [](std::stringstream &ss, content_replace &cr) {
        return do_with(
                   output_stream<char>(cr.transform(std::make_unique<nil::actor::httpd::request>(), "html",
                                                    output_stream<char>(memory_data_sink(ss), 32000, true))),
                   [](output_stream<char> &os) {
                       return os.write(sstring("hello-{{Protocol}}-xyz-{{Host}}")).then([&os] { return os.close(); });
                   })
            .then([&ss, &cr]() {
                BOOST_REQUIRE_EQUAL(ss.str(), "hello-{{Protocol}}-xyz-{{Host}}");
                return test_transformer_stream(ss, cr, {"hell", "o-{", "{Pro", "tocol}}-xyz-{{Ho", "st}}{{Pr"})
                    .then([&ss, &cr] {
                        BOOST_REQUIRE_EQUAL(ss.str(), "hello-http-xyz-localhost{{Pr");
                        return test_transformer_stream(
                                   ss, cr, {"hell", "o-{{", "Pro", "tocol}}{{Protocol}}-{{Protoxyz-{{Ho", "st}}{{Pr"})
                            .then([&ss, &cr] {
                                BOOST_REQUIRE_EQUAL(ss.str(), "hello-httphttp-{{Protoxyz-localhost{{Pr");
                                return test_transformer_stream(ss, cr,
                                                               {"hell", "o-{{Pro", "t{{Protocol}}ocol}}", "{{Host}}"})
                                    .then([&ss] { BOOST_REQUIRE_EQUAL(ss.str(), "hello-{{Prothttpocol}}localhost"); });
                            });
                    });
            });
    });
...
struct http_consumer {
    ...
    enum class status_type { READING_HEADERS, CHUNK_SIZE, CHUNK_BODY, CHUNK_END, READING_BODY_BY_SIZE, DONE };
            ...
            if (last == '\r' && c == '\n') {
                if (_current == "") {
                    if (status == status_type::READING_HEADERS || (status == status_type::CHUNK_BODY && _remain == 0)) {
                        if (status == status_type::READING_HEADERS &&
                    ...
                    switch (status) {
                        case status_type::READING_HEADERS:
                            add_header(_current);
                            break;
                        case status_type::CHUNK_SIZE:
                            set_chunk(_current);
                            break;
                        default:
                            break;
...
public:
    ...
    static future<> run_test(std::function<future<>(output_stream<char> &&)> &&write_func,
        ...
        return do_with(
            ...
            [reader, &write_func](loopback_connection_factory &lcf, auto &server) {
                return do_with(
                           loopback_socket_impl(lcf),
                           [&server, &lcf, reader, &write_func](loopback_socket_impl &lsi) {
                               httpd::http_server_tester::listeners(*server).emplace_back(lcf.get_server_socket());

                               auto client = nil::actor::async([&lsi, reader] {
                                   connected_socket c_socket =
                                       lsi.connect(socket_address(ipv4_addr()), socket_address(ipv4_addr())).get0();
                                   ...
                                   while (more) {
                                       ...
                                       repeat([&input, &htp] {
                                           return input.read().then([&htp](const temporary_buffer<char> &b) mutable {
                                               return (b.size() == 0 || htp.read(b)) ?
                                                          make_ready_future<stop_iteration>(stop_iteration::yes) :
                               ...
                               auto server_setup = nil::actor::async([&server, &write_func] {
                                   ...
                                   public:
                                       test_handler(http_server &server,
                                                    std::function<future<>(output_stream<char> &&)> &&write_func) :
                                           _server(server),
    ...
    static future<> run(std::vector<std::tuple<bool, size_t>> tests) {
        return do_with(
            ...
            [tests](loopback_connection_factory &lcf, auto &server) {
                return do_with(
                           loopback_socket_impl(lcf),
                           [&server, &lcf, tests](loopback_socket_impl &lsi) {
                               httpd::http_server_tester::listeners(*server).emplace_back(lcf.get_server_socket());

                               auto client = nil::actor::async([&lsi, tests] {
                                   connected_socket c_socket =
                                       lsi.connect(socket_address(ipv4_addr()), socket_address(ipv4_addr())).get0();
                                   input_stream<char> input(c_socket.input());
                                   output_stream<char> output(c_socket.output());
                               ...
                               auto server_setup = nil::actor::async([&server, tests] {
                                   class test_handler : public handler_base {
                                       size_t count = 0;
                                       http_server &_server;
                                   ...
                                   public:
                                       test_handler(http_server &server,
                                                    const std::vector<std::tuple<bool, size_t>> &tests) :
                                           _server(server),
                                           _tests(tests) {
                                       }
                                       future<std::unique_ptr<reply>> handle(const sstring &path,
    ...
    static noncopyable_function<future<>(output_stream<char> &&o_stream)> make_writer(size_t len, bool success) {
        return [len, success](output_stream<char> &&o_stream) mutable {
            return do_with(output_stream<char>(std::move(o_stream)), uint32_t(len / 10),
                           [success](output_stream<char> &str, uint32_t &remain) {
                               if (remain == 0) {
                                   if (success) {
                                       return str.close();
                                   } else {
...
SEASTAR_TEST_CASE(test_http_client_server_full) {
    std::vector<std::tuple<bool, size_t>> tests = {std::make_tuple(true, 100),  std::make_tuple(true, 10000),
                                                   ...
                                                   std::make_tuple(true, 9000), std::make_tuple(true, 10000)};
    return test_client_server::run(tests);
}
...
SEASTAR_TEST_CASE(content_length_limit) {
    return nil::actor::async([] {
        ...
        future<> client = nil::actor::async([&lsi] {
                ...
                for (auto content : {sstring(""), sstring("xxxxxxxxxxx")}) {
                    for (auto expect :
                        ...
                        auto content_len =
                            content.empty() ?
                                sstring("") :
                                (sstring("Content-Length: ") + to_sstring(content.length()) + sstring("\r\n"));
                        sstring req = sstring("GET /test HTTP/") + version +
                        ...
                        if (version == "1.1" && expect.length()) {
                            auto resp = input.read().get0();
                            BOOST_REQUIRE_NE(std::string(resp.get(), resp.size()).find("100 Continue"),
                                             std::string::npos);
                            already_ok = content.empty() &&
                                         std::string(resp.get(), resp.size()).find("200 OK") != std::string::npos;
                        }
                        if (!already_ok) {
                            ...
                            output.flush().get();
                            auto resp = input.read().get0();
                            BOOST_REQUIRE_EQUAL(std::string(resp.get(), resp.size()).find("100 Continue"),
                                                std::string::npos);
                            BOOST_REQUIRE_NE(std::string(resp.get(), resp.size()).find("200 OK"), std::string::npos);
                        }
                    }
