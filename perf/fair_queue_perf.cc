...
struct local_fq_and_class {
    ...
    nil::actor::fair_queue& queue(bool local) noexcept { return local ? fq : sfq; }

    local_fq_and_class(nil::actor::fair_group& sfg)
        : fg(nil::actor::fair_group::config(1, 1))
        , fq(fg, nil::actor::fair_queue::config())
        , sfq(sfg, nil::actor::fair_queue::config())
...
struct local_fq_entry {
    ...
    local_fq_entry(unsigned weight, unsigned index, Func&& f)
        : ent(nil::actor::fair_queue_ticket(weight, index))
        , submit(std::move(f)) {}
};
...
struct perf_fair_queue {
    ...
    perf_fair_queue()
        : shared_fg(nil::actor::fair_group::config(smp::count, smp::count))
    {
