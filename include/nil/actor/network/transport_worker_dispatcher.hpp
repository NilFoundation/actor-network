    ...
    namespace actor {
        namespace network {
            ...
            public:
                ...
                explicit transport_worker_dispatcher(factory_type factory) : factory_(std::move(factory)) {
                    // nop
                }
                ...
                error init(Parent &) {
                    ACTOR_ASSERT(workers_by_id_.empty());
                    return none;
                }
                ...
                void resolve(Parent &parent, const uri &locator, const actor &listener) {
                    if (auto worker = find_worker(make_node_id(locator)))
                        ...
                        anon_send(listener, make_error(sec::runtime_error, "could not resolve node"));
                }
                ...
                template<class Parent>
                void local_actor_down(Parent &parent, const node_id &nid, actor_id id, error reason) {
                    if (auto worker = find_worker(nid))
                ...
                template<class Parent>
                expected<worker_ptr> add_new_worker(Parent &parent, node_id node, id_type id) {
                    ACTOR_LOG_TRACE(ACTOR_ARG(node) << ACTOR_ARG(id));
            ...
            private:
                ...
                template<class Key>
                worker_ptr find_worker_impl(const std::unordered_map<Key, worker_ptr> &map, const Key &key) {
                    if (map.count(key) == 0) {
                        ACTOR_LOG_DEBUG("could not find worker: " << ACTOR_ARG(key));
                        return nullptr;
                    }
                    return map.at(key);
                }
