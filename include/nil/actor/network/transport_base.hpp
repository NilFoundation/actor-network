...
namespace nil {
    namespace actor {
        namespace network {
            ...
            /// of `NextLayer`.
            template<class Transport, class NextLayer, class Handle, class Application, class IdType>
            ...
            public:
                ...
                /// Returns a reference to the `spawner` of this transport.
                ...
                virtual error init(endpoint_manager &parent) {
                    ...
                    auto max_header_bufs =
                        get_or(cfg, "middleman.max-header-buffers", defaults::middleman::max_header_buffers);
                    header_bufs_.reserve(max_header_bufs);
                    auto max_payload_bufs =
                        get_or(cfg, "middleman.max-payload-buffers", defaults::middleman::max_payload_buffers);
                ...
                auto resolve(endpoint_manager &, const uri &locator, const actor &listener) {
                    ACTOR_LOG_TRACE(ACTOR_ARG(locator) << ACTOR_ARG(listener));
                    auto f = detail::make_overload(
                        [&](auto &layer) -> decltype(layer.resolve(*this, locator, listener)) {
                            return layer.resolve(*this, locator, listener);
                        },
                        [&](auto &layer) -> decltype(layer.resolve(*this, locator.path(), listener)) {
