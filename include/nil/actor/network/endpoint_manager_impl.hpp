    ...
    namespace actor {
        namespace network {
            ...
            public:
                ...
                endpoint_manager_impl(const multiplexer_ptr &parent, spawner &sys, Transport trans) :
                ...
                template<class... Ts>
                uint64_t set_timeout(actor_clock::time_point tp, std::string type, Ts &&... xs) {
                    auto act = actor_cast<abstract_actor *>(timeout_proxy_);
                    ACTOR_ASSERT(act != nullptr);
                            ...
                            for (auto ptr = q.next(); ptr != nullptr; ptr = q.next()) {
                                auto f = detail::make_overload(
                                    ...
                                    [&](endpoint_manager_queue::event::local_actor_down &x) {
                                        transport_.local_actor_down(*this, x.observing_peer, x.id, std::move(x.reason));
                                    },
