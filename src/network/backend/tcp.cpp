...
#include <nil/actor/network/basp/application_factory.hpp>
    ...
    namespace actor {
        namespace network {
            namespace backend {
                ...
                tcp::tcp(middleman &mm) : middleman_backend("tcp"), mm_(mm), proxies_(mm.system(), *this) {
                    // nop
                }
                ...
                error tcp::init() {
                    uint16_t conf_port =
                        get_or<uint16_t>(mm_.system().config(), "middleman.tcp-port", defaults::middleman::tcp_port);
                    ip_endpoint ep;
                    ...
                    auto mgr = make_endpoint_manager(
                        mpx, mm_.system(), doorman {acc_guard.release(), basp::application_factory {proxies_}});
