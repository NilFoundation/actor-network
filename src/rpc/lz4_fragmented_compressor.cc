...
namespace nil {
    namespace actor {
        namespace rpc {
            ...
            std::unique_ptr<rpc::compressor> lz4_fragmented_compressor::factory::negotiate(sstring feature,
            ...
            snd_buf lz4_fragmented_compressor::compress(size_t head_space, snd_buf data) {
                static thread_local auto stream =
                    std::unique_ptr<LZ4_stream_t, compression_stream_deleter>(LZ4_createStream());
                ...
                if (single_chunk_size <= chunk_size && size_left <= chunk_size && src->size() == size_left) {
                    ...
                    auto compressed_size = LZ4_compress_fast_continue(stream.get(), src->get(), compressed_data,
                                                                      size_left, LZ4_COMPRESSBOUND(size_left), 0);
                ...
                static_assert(chunk_maximum_compressed_size < snd_buf::chunk_size,
                              "chunk_maximum_compressed_size is too large");
                ...
                while (src_left > chunk_size) {
                    ...
                    auto compressed_size =
                        LZ4_compress_fast_continue(stream.get(), src_ptr, dst, chunk_size, chunk_compress_bound, 0);
                ...
                auto compressed_size =
                    LZ4_compress_fast_continue(stream.get(), src_ptr, dst, rem, last_chunk_compress_bound, 0);
                dst_offset += compressed_size + chunk_header_size;
            ...
            rcv_buf lz4_fragmented_compressor::decompress(rcv_buf data) {
                ...
                static thread_local auto stream =
                    std::unique_ptr<LZ4_streamDecode_t, decompression_stream_deleter>(LZ4_createStreamDecode());
                ...
                auto read_header = [&] {
                    ...
                    return boost::endian::little_to_native(header_value);
                };
                ...
                if (src) {
                    ...
                    if (header & last_chunk_flag) {
                        ...
                        if (LZ4_decompress_safe_continue(stream.get(), src->get() + src_offset, dst.get_write(),
                                                         src_left, header) < 0) {
                ...
                while (!(header_value & last_chunk_flag)) {
                    ...
                    if (LZ4_decompress_safe_continue(stream.get(), src_ptr, /*dst_buffers.back().get_write()*/ dst,
                                                     header_value, chunk_size) < 0) {
                        throw std::runtime_error(
                            format("RPC frame LZ4_FRAGMENTED decompression failure (long, at {} bytes)",
                                   total_size - chunk_size));
                    }
                ...
                if (LZ4_decompress_safe_continue(stream.get(), src_ptr, /*dst_buffers.back().get_write()*/ dst,
                                                 last_chunk_compressed_size, header_value) < 0) {
                    throw std::runtime_error(
                        format("RPC frame LZ4_FRAGMENTED decompression failure (long, last frame, at {} bytes)",
                               total_size - header_value));
                }
